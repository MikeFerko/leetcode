#
# @lc app=leetcode id=435 lang=python3
#
# [435] Non-overlapping Intervals
#
# https://leetcode.com/problems/non-overlapping-intervals/description/
#
# algorithms
# Medium (57.07%)
# Likes:    9335
# Dislikes: 262
# Total Accepted:    1.1M
# Total Submissions: 1.9M
# Testcase Example:  '[[1,2],[2,3],[3,4],[1,3]]'
#
# Given an array of intervals intervals where intervals[i] = [starti, endi],
# return the minimum number of intervals you need to remove to make the rest of
# the intervals non-overlapping.
# 
# Note that intervals which only touch at a point are non-overlapping. For
# example, [1, 2] and [2, 3] are non-overlapping.
# 
# 
# Example 1:
# 
# 
# Input: intervals = [[1,2],[2,3],[3,4],[1,3]]
# Output: 1
# Explanation: [1,3] can be removed and the rest of the intervals are
# non-overlapping.
# 
# 
# Example 2:
# 
# 
# Input: intervals = [[1,2],[1,2],[1,2]]
# Output: 2
# Explanation: You need to remove two [1,2] to make the rest of the intervals
# non-overlapping.
# 
# 
# Example 3:
# 
# 
# Input: intervals = [[1,2],[2,3]]
# Output: 0
# Explanation: You don't need to remove any of the intervals since they're
# already non-overlapping.
# 
# 
# 
# Constraints:
# 
# 
# 1 <= intervals.length <= 10^5
# intervals[i].length == 2
# -5 * 10^4 <= starti < endi <= 5 * 10^4
# 
# 
#

# @lc code=start
from typing import List

class Solution:
    def eraseOverlapIntervals(self, intervals: List[List[int]]) -> int:
        """
        Given an array of intervals `intervals` where `intervals[i] = [starti, endi]`,
        return the minimum number of intervals you need to remove to make the 
        rest of the intervals non-overlapping.

        NOTE that intervals which only touch at a point are non-overlapping.
        For example, [1, 2] and [2, 3] are non-overlapping.

        Approach:
            1. Sort the intervals by end.
            2. Iterate through the intervals and check if the start is greater than or equal to the previous end.
            3. If it is, update the previous end to the current end.
            4. If it is not, increment the count for the number of intervals to remove.
            5. Return the count.

        Args:
            intervals (List[List[int]]): Array of intervals where intervals[i] = [starti, endi]

        Returns:
            int: Minimum number of intervals to remove
        
        @complexity: Time: O(nlogn)
        @complexity: Space: O(1)
        """
        intervals.sort(key=lambda x: x[1]) # sort by end O(nlogn)
        count = 0 # count of intervals to remove
        prev = float('-inf')
        for start, end in intervals:
            if start >= prev:
                prev = end
            else:
                # increment the count. This is where interval removal would occur.
                count += 1
        return count

# @lc code=end

